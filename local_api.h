...
private:
  ...
  void handleHtml_() {
    ...
    h += htmlRow_("Internal flash storage", sdOk ? "ok" : "absent");
  ...
  // ---- storage presence (§5: "reusing existing SD.begin()/SD.exists()",
  // now LittleFS on internal flash -- no SD card on this unit) ------------
  bool sdPresent_() {
    // Cheap, non-disruptive: LittleFS.exists() does not remount. /queue is
    // guaranteed to exist once LittleFS.begin() has ever succeeded this boot
    // (queue.h::begin() creates it) -- present/absent only, per §5's scope
    // note; the full degradation state machine is ADR-004's job.
    return LittleFS.exists(SD_QUEUE_DIR);
  }
