...
class Adr001SchemaAcceptanceTests(unittest.TestCase):
    ...
    def test_current_schema_accepted(self):
        resp = self._push(
            [
                {
                    "schema_version": server.CURRENT_SCHEMA_VERSION,
                    "record_type": 1,
                    "boot_id": 1,
                    "seq": 1,
                    "ts": 100,
    ...
    def test_previous_schema_accepted_once_registered(self):
        orig = (
            ...
            server.PREVIOUS_SCHEMA_VERSION,
    ...
    def test_unsupported_version_quarantined_alongside_accepted(self):
        ...
        self.assertEqual(resp.get_json()["ack_seq"], 2)
        self.assertEqual(
            resp.get_json()["quarantined"], [{"seq": 2, "reason": "unsupported_schema_version:7"}]
        )
    ...
    def test_no_crash_on_malformed_or_missing_schema(self):
        ...
        malformed_batches = [
            ...
            [
                {"record_type": 1, "boot_id": 1, "seq": 2, "ts": 1, "totalizer": 1}
            ...
            [
                {"schema_version": 1, "record_type": 1, "boot_id": 1, "seq": 4}
