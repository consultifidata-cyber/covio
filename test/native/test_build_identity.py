...
from build_identity import BuildIdentityError, resolve_build_identity  # noqa: E402
...
class BuildIdentityNonReleaseTests(unittest.TestCase):
    ...
    def test_clean_resolvable_sha_reported_exactly(self):
        commit, dirty, _ = resolve_build_identity("esp32dev", lambda: GOOD_SHA, lambda: False, _now)
        self.assertEqual(commit, GOOD_SHA)
        self.assertFalse(dirty)

    def test_dirty_tree_reported_not_raised(self):
        commit, dirty, _ = resolve_build_identity("esp32dev", lambda: GOOD_SHA, lambda: True, _now)
        self.assertEqual(commit, GOOD_SHA)
        self.assertTrue(dirty)

    def test_missing_git_falls_back_without_raising(self):
        commit, dirty, _ = resolve_build_identity("esp32dev", lambda: None, lambda: False, _now)
        self.assertEqual(commit, "dev-nogit")

    def test_malformed_sha_falls_back_without_raising(self):
        commit, _, _ = resolve_build_identity("factory", lambda: "garbage", lambda: False, _now)
        self.assertEqual(commit, "dev-nogit")
    ...
    def test_unknown_dirty_status_defaults_true_not_silently_clean(self):
        ...
        _, dirty, _ = resolve_build_identity("esp32dev", lambda: GOOD_SHA, lambda: None, _now)
        self.assertTrue(dirty)

    def test_build_time_is_utc_and_machine_readable(self):
        _, _, build_time = resolve_build_identity("esp32dev", lambda: GOOD_SHA, lambda: False, _now)
