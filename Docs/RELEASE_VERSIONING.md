...
|---|---|---|
| Firmware binary — **application** | `<product>-vX.Y.Z-app.bin` | `<product>` is `covio-oilflow` (from `[env:release]`) or `miki-wire` (from `[env:release-mikiwire]`), both `-DRELEASE_BUILD=1`. **This is the file a technician flashes**, at offset `0x10000`. The product-qualified naming this row now uses is exactly the switch the previous single-target version of this table anticipated ("the pattern to switch to if/when a second board target is ever added") — that day arrived when the Miki Wire profile shipped. **Both products are required in every release**; `release.yml` refuses to publish with either missing. |
| Firmware binary — **factory image** | `<product>-vX.Y.Z-FACTORY-ONLY-full.bin` | A `merge_bin` image written at `0x0`, for **virgin units only**. ⚠ It pads the gap between the partition table and `boot_app0` with `0xFF`, and the NVS partition sits in exactly that gap (`0x9000`–`0xDFFF`) — so flashing it to a commissioned unit erases the API key, WiFi, server URL, totalizer checkpoint and OTA anti-downgrade floor, and a `RELEASE_BUILD` image with no API key halts at boot and stops metering. The `FACTORY-ONLY` in the filename is a warning, not a label. |
| Firmware debug symbols (ELF) | `<product>-vX.Y.Z.elf` | The unstripped linker output — required for post-mortem crash/backtrace analysis against a `.bin` running in the field. Always produced by a successful build (standard GCC/PlatformIO toolchain behavior) — **required**, same as `.bin`. Added by RE-7. |
| Firmware linker map | `<product>-vX.Y.Z.map` | Best-effort only — not every toolchain/framework combination emits a `.map` file without an explicit linker flag this project has not added (`Docs/FIRMWARE_BUILD.md`). Included in the release when produced; its absence is a warning, never a failure, and never blocks a release. Added by RE-7. |
| Bootloader / partition table / boot_app0 | `<product>-vX.Y.Z-{bootloader,partitions,boot_app0}.bin` | The per-offset images, published so an on-site engineer can rebuild the flash by hand without re-deriving offsets. Routine updates need none of these — only `-app.bin` at `0x10000`. |
| "Which file do I flash?" | `WHICH-FILE-DO-I-FLASH.txt` | Generated at build time and attached to the Release. Ships **with** the binaries deliberately: the person reading it is standing at a machine, and a copy that lives only in the repo is a copy that goes stale. |
